<template>
	<view class="container" v-if="showTip">
		<view class="uni-add-tips-box" :style="{ top : (capsuleInfo.top+capsuleInfo.height) + 'px'}">
	...
	export default{
		...
		mounted() {
			wx.checkIsAddedToMyMiniProgram({
			          success: (res) => {
						  this.isAddedToMymp = res.added;
			          },
			          fail: () => console.error('检测失败')
			        });
			
			this.showTip = !this.isAddedToMymp && !uni.getStorageInfoSync().keys.includes(SHOW_TIP)
